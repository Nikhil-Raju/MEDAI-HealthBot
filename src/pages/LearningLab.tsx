...
import { BookOpen, ChevronLeft, ChevronRight, CheckCircle, HelpCircle } from "lucide-react";
...
const MODULES = [
  {
    ...
    lessons: [
      {
        title: "Heart Anatomy",
        content: "Learn about the structure of the human heart, its chambers, and valves.",
        diagram: "/public/diagrams/heart-3d.png",
      },
      {
        title: "Blood Flow",
        content: "Understand the flow of blood through the heart and body.",
        diagram: "/public/diagrams/blood-flow-3d.png",
      },
    ],
    quiz: [
      {
      ...
      },
      {
        ...
        options: ["Septum", "Valve", "Atrium", "Ventricle"],
        answer: "Septum",
      },
    ],
  },
  {
    ...
    lessons: [
      {
        title: "Brain Regions",
        content: "Explore the major regions of the brain and their functions.",
        diagram: "/public/diagrams/brain-3d.png",
      },
      {
        title: "Neural Pathways",
        content: "Discover how neurons communicate and form networks.",
        diagram: "/public/diagrams/neural-pathways-3d.png",
      },
    ],
    quiz: [
      {
      ...
      },
      {
        ...
        options: ["Neuron", "Glia", "Axon", "Dendrite"],
        answer: "Neuron",
      },
    ],
  },
...
export default function LearningLab() {
  ...
  const [quizOrder, setQuizOrder] = useState(shuffleArray(MODULES[0].quiz));
  ...
  const handleModuleSelect = (idx: number) => {
    ...
    setLessonIndex(0);
    setShowQuiz(false);
    setQuizOrder(shuffleArray(MODULES[idx].quiz));
    setQuizIndex(0);
  ...
  const handleNextLesson = () => {
    if (lessonIndex < MODULES[selectedModule].lessons.length - 1) {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 flex flex-col items-center px-2 pb-8 pt-4">
      <header className="w-full max-w-2xl mx-auto flex items-center gap-2 mb-4">
        <BookOpen className="text-blue-600 w-7 h-7" />
        <h1 className="font-inter font-bold text-2xl text-blue-700 tracking-tight">Learning Lab</h1>
      </header>
      {/* Module Selector */}
      <motion.div
        className="w-full max-w-2xl overflow-x-auto flex gap-3 pb-2 snap-x snap-mandatory"
      ...
      >
        {MODULES.map((mod, idx) => (
          <button
            key={mod.title}
            className={`snap-start flex-shrink-0 px-4 py-2 rounded-xl shadow-md font-semibold transition-all duration-200 whitespace-nowrap
              ${selectedModule === idx
                ? "bg-blue-600 text-white scale-105 shadow-lg"
                : "bg-white text-blue-700 hover:bg-blue-50"}
            `}
            onClick={() => handleModuleSelect(idx)}
            aria-current={selectedModule === idx}
          >
            {mod.title}
          </button>
        ))}
      ...
      {/* Lesson/Quiz Card */}
      <motion.div
        className="w-full max-w-md mt-6 bg-white rounded-2xl shadow-lg p-4 flex flex-col items-center relative min-h-[340px]"
      ...
      >
        {!showQuiz ? (
          <>
            <div className="w-full flex items-center justify-between mb-2">
              <span className="text-xs text-neutral-400 font-medium">
                Lesson {lessonIndex + 1} of {MODULES[selectedModule].lessons.length}
              </span>
              <span className="text-xs text-blue-500 font-semibold">
                {MODULES[selectedModule].title}
              </span>
            </div>
            <h2 className="font-inter font-bold text-lg text-blue-700 mb-1">
              {MODULES[selectedModule].lessons[lessonIndex].title}
            </h2>
            <p className="text-neutral-700 text-base mb-3 text-center">
              {MODULES[selectedModule].lessons[lessonIndex].content}
            </p>
            {/* 3D Diagram */}
            <motion.img
              src={MODULES[selectedModule].lessons[lessonIndex].diagram}
              alt="3D Diagram"
              className="w-full max-w-[220px] h-auto rounded-xl shadow aspect-video object-cover mb-4 border border-blue-100"
            ...
            <div className="flex w-full justify-between mt-auto pt-2">
              <button
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-neutral-100 text-blue-600 font-semibold disabled:opacity-40"
              ...
              <button
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-blue-600 text-white font-semibold shadow-md"
                onClick={handleNextLesson}
              >
                {lessonIndex < MODULES[selectedModule].lessons.length - 1 ? (
                  <>
          ...
          <>
            ...
            <h2 className="font-inter font-bold text-lg text-blue-700 mb-2 text-center">
              {quizOrder[quizIndex].question}
            </h2>
            <div className="flex flex-col gap-2 w-full mt-2">
              {quizOrder[quizIndex].options.map((opt) => (
                <button
                  key={opt}
                  className={`w-full px-4 py-2 rounded-lg border text-base font-medium transition-all duration-150 flex items-center justify-between
                    ${selectedOption
                      ? opt === quizOrder[quizIndex].answer
                        ? "bg-green-100 border-green-400 text-green-700"
                        : opt === selectedOption
                        ? "bg-red-100 border-red-400 text-red-700"
                        : "bg-neutral-50 border-neutral-200 text-neutral-700"
                      : "bg-neutral-50 border-neutral-200 hover:bg-blue-50 hover:border-blue-300"}
                  `}
                  onClick={() => !selectedOption && handleOptionSelect(opt)}
                  disabled={!!selectedOption}
                >
                  {opt}
                  {selectedOption && opt === quizOrder[quizIndex].answer && (
          ...
          <div className="flex flex-col items-center justify-center w-full h-full min-h-[220px]">
            ...
            <h2 className="font-inter font-bold text-xl text-blue-700 mb-1">Quiz Complete!</h2>
            <p className="text-neutral-700 text-base mb-2">
              You scored {correctCount} out of {quizOrder.length}.
            </p>
            <button
              className="mt-2 px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold shadow-md"
              onClick={() => {
                setShowQuiz(false);
                setLessonIndex(0);
                setQuizCompleted(false);
                ...
                setSelectedOption(null);
                setCorrectCount(0);
                setQuizOrder(shuffleArray(MODULES[selectedModule].quiz));
              }}
            >
              Restart Module
            </button>
      ...
      <div className="w-full max-w-2xl flex justify-center mt-4 md:hidden">
        <div className="flex gap-1">
          {MODULES.map((_, idx) => (
            <span
              key={idx}
              className={`block w-3 h-3 rounded-full transition-all duration-200
                ${selectedModule === idx ? "bg-blue-600 scale-110" : "bg-blue-200"}
              `}
            />
          ))}
        </div>
...
// ...existing code up to the end of the correct LearningLab component...
    </div>
  );
}
import React, { useState } from "react";
...
export default function LearningLab() {
  ...
  // Responsive horizontal scroll for modules
  // Tailwind: overflow-x-auto, flex-nowrap, snap-x, snap-mandatory
...
const MODULES = [
  {
    ...
    description: "Learn the basics of how AI is transforming healthcare.",
    content:
      "Artificial Intelligence (AI) is revolutionizing healthcare by enabling faster diagnoses, personalized treatments, and improved patient outcomes.",
  },
  {
    id: 2,
  ...
  },
  {
    ...
    description: "Explore real-world applications of AI in healthcare.",
    content:
      "AI is used in medical imaging, drug discovery, patient monitoring, and administrative tasks to enhance efficiency and accuracy.",
  },
];
...
function LearningModuleCard({
  ...
  return (
    <button
      className={`min-w-[260px] max-w-xs flex-shrink-0 rounded-xl border transition-colors px-4 py-3 mr-4 mb-3 text-left shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 ${
        isSelected
...
export default function LearningLab() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold mb-2 text-blue-700">Learning Lab</h1>
      <p className="mb-6 text-slate-600 text-sm">
        Welcome to the AI in Healthcare Learning Lab! Select a module to get started.
      </p>
      <div className="overflow-x-auto pb-2 -mx-4">
        <div className="flex flex-row gap-0 px-4">
          {MODULES.map((module) => (
            <LearningModuleCard
              key={module.id}
              module={module}
      ...
      {selectedModule && (
        <div className="mt-6 bg-white rounded-xl border border-slate-200 shadow p-5">
          <h2 className="text-lg font-bold mb-2 text-blue-600">{selectedModule.title}</h2>
          <p className="text-slate-700 text-base">{selectedModule.content}</p>
        </div>
      )}
    </div>
  );
